...
Select a.inscl , a.namepttype,   
    
count(CASE WHEN a.yeaript between '$year5' and '$year5_1'  THEN   a.can END) AS 'aaa' ,
count(CASE WHEN a.yeaript between '$year4' and '$year4_1'  THEN  a.can END) AS 'bbb' ,
 count(CASE WHEN a.yeaript between '$year3' and '$year3_1' THEN  a.can END) AS 'ccc' ,
count(CASE WHEN a.yeaript between '$year2' and '$year2_1'  THEN  a.can END) AS 'ddd' 

    from(
SELECT  ovst.vn as can, 
	ovst.hn, 
	ovst.pttype, 
	date(ovst.vstdttm) as yeaript, 
	pttype.namepttype, 
	pttype.inscl
   
FROM pttype INNER JOIN ovst ON pttype.pttype = ovst.pttype
WHERE DATE_FORMAT(ovst.vstdttm,'%Y-%m-%d') between '$year5' and '$year2_1' and pttype.pttype not in(15,10,35,14,11)  ) as a GROUP BY  a.inscl order by ddd desc";
