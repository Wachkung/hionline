...
$query="select a.rfrlct,a.off_name1,
       
SUM(CASE WHEN a.accdate between '2010-10-01'and '2011-09-30' THEN a.can END) AS aaa,
SUM(CASE WHEN a.accdate between '2011-10-01'and '2012-09-30' THEN a.can END) AS bbb,
 SUM(CASE WHEN a.accdate between '2012-10-01'and '2013-09-30' THEN a.can END) AS ccc ,
SUM(CASE WHEN a.accdate between '2013-10-01'and '2014-09-30' THEN a.can END) AS ddd

from( SELECT count(orfri.vn ) as can,
	...
	hospcode.off_name1,
     orfri.vn
FROM ovstdx INNER JOIN orfri ON ovstdx.vn = orfri.vn
	 INNER JOIN hospcode ON hospcode.off_id = orfri.rfrlct
	 INNER JOIN icd101 ON icd101.icd10 = ovstdx.icd10
WHERE year(orfri.accdate) between'2010' and '2014'   and ovstdx.cnt='1'   group by vn  )as a  group by  a.rfrlct  ORDER BY ddd DESC
";
