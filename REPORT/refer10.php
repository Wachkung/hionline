...
WHERE
date(ovst.vstdttm) BETWEEN '$startDate' AND '$endDate'  and
ovst.ovstost = 3  
GROUP BY  cln ORDER BY  cvn  desc  limit 10";
...
<?php
 ...
 while ($row2 = mysql_fetch_array($resultpt10, MYSQL_ASSOC)) {
		$data2[] = array(
			      ...
			      'cln' => $row2['cln'],
			      'namecln' => iconv('TIS-620','UTF-8//IGNORE',$row2['namecln']) ,
			//    'FNAME' => 
		 	    'cvn'=>  iconv('TIS-620','UTF-8//IGNORE',$row2['cvn']) 
		  );
	}
