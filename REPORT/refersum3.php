...
$query="select a.idpm,a.nameidpm,
...
SUM(CASE WHEN a.yeaript between '2013-10-01'and '2014-09-30' THEN a.can END) AS 'ddd' 
    ...
    ipt.dchdate as yeaript,
     idpm.nameidpm as nameidpm,
     idpm.idpm as idpm,
	count(ipt.an) as can
FROM idpm INNER JOIN ipt ON idpm.idpm = ipt.ward
where year(ipt.dchdate) between '2010'and '2014' and ipt.dchtype ='4'   GROUP BY idpm, yeaript)  as a GROUP BY a.idpm
";
